...
def validate_continuation(parent, days=None):
    ...
    if man.get("input_fingerprints") and man["input_fingerprints"] != input_fingerprints(cfg):
        raise ValueError("Continuation requires unchanged population and initial-memory files")
    ...
    if total * tpd < boundary or (total * tpd == boundary and man.get("status") != "paused"):
        raise ValueError("--days is the total horizon and must extend beyond the committed parent history")
    return cfg, boundary, total
...
def continue_run(parent, out, days=None, *, progress=True):
    ...
    cfg, boundary, total = validate_continuation(parent, days)
