"""LEGACY (v1) latent-event scenario scripts, kept verbatim for the pre-v2 engine path and for
reproducing runs made before ontology v2. v2 runs use the pre-generated world script
(world_script.py) built from structure schemas x surface skins (structures.py, skins/).
...
`EventInstance` and `LATENT_TYPES` live in latent_events.py and are imported lazily here, so either
...
def assign_home_groups(families: list[str], groups: dict, rng) -> dict:
    """Each hidden family gets a 'home' friend group (SIMULATOR-ONLY, D46): instances of that family
    mostly happen to its members, so the same kind of thing keeps happening within one circle."""
    names = sorted(g for g, m in groups.items() if len(m) >= 2)
    order = [names[i] for i in rng.permutation(len(names))]
    return {f: order[i % len(order)] for i, f in enumerate(sorted(families))}


def instantiate(scn: dict, event_id: str, tick: int, rng, agents: dict, busy: set, tpd: int,
                prefer: list | None = None, prefer_prob: float = 0.0) -> EventInstance | None:
    ...
    `prefer`: the family's home group; with prob `prefer_prob` the protagonist (and, where possible,
    the related person) are cast from it.
    """
    from backend.simulation.latent_events import EventInstance
    ...
    if day_tick + max_off >= tpd - 1:
        return None
    home = sorted(a for a in (prefer or []) if a in free)
    use_home = bool(home) and rng.random() < prefer_prob
    pool = home if use_home else free
    ...
    if "{S}" in text_all:
        ...
        in_home = [o for o in rel if o in home]
        if use_home and in_home:
            rel = in_home
